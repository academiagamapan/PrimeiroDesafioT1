...
import desafio.Modelos.Cliente;
import desafio.Modelos.Compra;
import desafio.Modelos.Produto;
import desafio.Utilitaries.Utilidades;
...
public class Main {
     ...
     static TreeSet<Produto> estoque = Utilidades.getProdutos();
    ...
    private static void mostrarEstoque() {
        ...
        do{
            ...
            for(Produto p : estoque){
                System.out.printf("%15s%5s%d%8s%"+Utilidades.tamanhoMaiorString(estoque)+"s%9d%6sR$%.2f\n",esp,esp,
    ...
    private static void mostrarNotaFiscal(int i) throws InterruptedException {
        ...
        for(Produto p : cliente.getCompras().getProdutos()){
            System.out.printf("%10s%-10d%"+Utilidades.tamanhoMaiorString(cliente.getCompras().getProdutos())+

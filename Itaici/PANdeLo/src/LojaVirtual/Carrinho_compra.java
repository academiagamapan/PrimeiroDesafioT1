...
public class Carrinho_compra {
	public static void main(String[] args) {
		...
		if (!carrinho.isEmpty()) {
			...
			do {
				...
				System.out.println("2 - Cartão à vista (a partir de 10% de desconto).");
	...
	public static boolean alterarQuantidade(ArrayList<Produto> carrinhos, ArrayList<Produto> produtos) {
		...
		for (Produto produto : carrinhos) {
			if (produto.getCodigo() == i) {
				ver = 1;
				System.out.print("Estoque disponível: " + produtos.get(i - 1).getQuantidade());
				int quantidade = 0;
				do {
					System.out.printf("\nDigite a nova quantidade: ");
					quantidade = (new Scanner(System.in)).nextInt();
					if (quantidade > produtos.get(i - 1).getQuantidade() + produto.getQuantidade()) {
						System.out.println("Quantidade indisponivel");
					}
				} while (quantidade > produtos.get(i - 1).getQuantidade() + produto.getQuantidade());

				produtos.get(i - 1)
						.setQuantidade(produtos.get(i - 1).getQuantidade() + produto.getQuantidade() - quantidade);
